...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/dist-diff2">
        <html>
            <head>
                <title>Dist-diff2
                    <xsl:value-of select="version"/> report
                </title>
                <script>
    ...
    <xsl:template match="folder">
        <xsl:variable name="TR-CLASS">
            <xsl:choose>
                ...
                <xsl:when test="@status = 'DIFFERENT'">different<xsl:if test="permissionDiffOnly='true'">_permissions</xsl:if></xsl:when>
    ...
    <xsl:template match="file|jar|class|archive">
        ...
        <xsl:if test="not(string-length(textDiff)=0) or rpm-details or jarDiff or permissionDiff">
            <tr class="{$TR-CLASS}">
                <td colspan="1"></td>
                <td colspan="4">
                    ...
                    <!-- if there are RPM details to be shown...-->
                    <xsl:apply-templates select="rpm-details"/>
    ...
    <xsl:template match="rpm-details">
        <div>
            <xsl:variable name="ELEMENT_ID_RPMDETAILS" select="generate-id(.)"/>
            <div>
                <button name="RPM details" id="{$ELEMENT_ID_RPMDETAILS}-toggle"
                        onclick="toggleVisibility('{$ELEMENT_ID_RPMDETAILS}');">Show RPM details
                </button>
            </div>
            <div id="{$ELEMENT_ID_RPMDETAILS}" style="display:none">
                <xsl:if test="@packageName">
                    <b>Package long name:</b>
                    <xsl:value-of select="@packageName"/>
                    <br/>
                </xsl:if>
                <xsl:if test="@packageShortName">
                    <b>Package short name:</b>
                    <xsl:value-of select="@packageShortName"/>
                    <br/>
                </xsl:if>
                <xsl:if test="available-package-versions/version">
                    <b>Available versions of this package:</b>
                    <br/>
                    <xsl:for-each select="available-package-versions/version">
                        <xsl:value-of select="."/>
                        <br/>
                    </xsl:for-each>
                    <br/>
